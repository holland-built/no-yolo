...
Stage scope: `git add skills/ docs/ hooks/ README.md .gitignore setup.sh settings.example.json SHIP.md CLAUDE.md memory/bin/ verify.sh verify-selftest.sh` (explicit paths: do NOT rely on a `*.md` shell glob, which expands in the CWD rather than the repo root. `memory/bin/` only; the rest of `memory/` is a Guard. `verify.sh` and `verify-selftest.sh` are tracked and CI runs them. They were omitted from this scope once, so a fix to the verifier itself silently never shipped.)
