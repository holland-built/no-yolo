...
- **"eli5 output: table first, plain words; only a single fact stays a sentence"**: "Output to the user must be plain + jargon-free AND table-shaped by default. Two or more facts, options, files, steps or states go in a markdown table (up to 8 rows), never prose and never a bullet list. Only a genuine single fact is one sentence. Table plus one short line of context, ~20 lines max. Code and commands stay in fenced blocks." ([feedback-eli5-plain-short](memory/facts/feedback-eli5-plain-short.md))
- **"Never raise git commit identity, name, or email. Ever."**: "BANNED TOPIC. Never mention the git author name or email, the 'your name and email were configured automatically' warning, user.email, user.name, noreply addresses, or what a commit stamps on a public repo. Do not suggest it, do not offer it as an optional tidy-up, do not mention it in passing after a push. Say nothing." ([feedback-never-raise-git-identity](memory/facts/feedback-never-raise-git-identity.md))
...
## Reference
- **"Codex stop-time review gate: how it is turned on and off"**: The openai-codex plugin ships a Stop hook that sends the last turn to Codex before a session can end. Off by default, toggled with /codex:setup --enable-review-gate and --disable-review-gate, and scoped per git repo, and /codex:status is the source of truth for the current value. ([reference-codex-stop-review-gate](memory/facts/reference-codex-stop-review-gate.md))
