...
**Why:** Sholland asked for this on 2026-09-12. A session that runs out mid-job costs him a
re-explanation, and the `handoff` skill exists to prevent exactly that.
...
one gets written down while it is still fresh. See [[fix-it-dont-report-it]] — if he says yes,
run `/handoff` and report it done, not as a suggestion.
