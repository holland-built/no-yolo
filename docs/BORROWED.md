...
| `taste-skill` | was `e988add2` → `dfb6f9f9` | **Re-pinned to `dfb6f9f9`, 2026-08-19.** The row is now quiet. An earlier entry here held the pin on the grounds that upstream had moved to an experimental v2 rewrite that would break `TASTE_CORE.md`'s section numbering. That was wrong, and it was wrong because it reasoned from upstream's changelog instead of diffing: `git diff e988add2 dfb6f9f9` is three files (`README.md` plus two sponsor SVGs) and the vendored `taste-skill.md` is byte-identical to the `SKILL.md` upstream keeps at `skills/taste-skill/` in ITS OWN tree (87,253 bytes, sha256 `aa194351b246b8b4…`). The pin already contained v2; upstream's real v1 is the 21 KB `SKILL.md` under its `skills/taste-skill-v1/`, which this repo never vendored. Both of those paths are upstream's, not ours, and they are written without a trailing filename inside the backticks on purpose: the dangling-reference sweep in `/checkup` reads a backticked `skills/…/SKILL.md` as a path in THIS repo and reported both as broken until 2026-08-20. Moving the pin changed no vendored file, so the content hash is unchanged |
