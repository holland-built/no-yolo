...
- **Fake precision:** any number, percentage, duration, or benchmark that was not actually measured: "~40% faster", "saves about 3 hours", "handles 10k requests/sec", a chart axis, a made-up date, a placeholder cost. **This is the worst tell in this file.** The tell is not the number's size, it is that nothing behind it was run. Full rule, and the day it was broken three times: `CORE_RULES.md` rule 1.
- **Overclaimed completion:** "done", "fixed", "should work now", "fully working", "production-ready" when the code was not run, the test was not executed, or the page was not opened. The tell is a completion claim with no evidence sentence next to it. Full rule: `CORE_RULES.md` rule 2.
...
- **Em-dash, at all.** Zero. The character `—` is banned outright, and so is `–` used as a separator. Headlines, body copy, bullets, captions, commit messages, chat replies, code comments: everywhere. Replace it with a full stop, a comma, a colon, parentheses, or a rewritten sentence. Ranges use a plain hyphen (`2018-2026`, `40-80k`). **One carve-out:** a `—` inside backticks or a code block is a *specimen*, quoting the character so the rule can name it, the way a style guide quotes the word it bans. Those stay. Anything the reader meets as ordinary prose does not. This is binary by owner's ruling, 2026-08-19, so it cannot be argued down. Why this stands, what the 2026-08-19 sweep measured, and what is still outstanding: `~/.claude/docs/DECISIONS.md`.
