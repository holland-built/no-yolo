...
- **slop-guard.js** — Stop; **blocks once, then warns.** Scans the finished reply against `hooks/slop-patterns.json` (edit that file, not the script) and names any listed phrase it finds. A match sends the reply back to be rewritten; on the rewrite pass Claude Code sets `stop_hook_active`, so it only warns and the turn can always finish — one false positive costs one retry, never a loop. `SLOP_GUARD=warn` restores warn-only. Exit status stays 0 in every path: Stop hooks block via stdout JSON, and a non-zero exit would read as a crashed hook. See the honesty note below — what it *can* see is still only a phrase list.
...
- **catalog-selfheal.js** — PostToolUse (Edit/Write/MultiEdit); after an edit to a `skills/*/SKILL.md` or one of the my-skills catalog sources, runs `regen.py --check` and, only if something drifted, re-runs `regen.py` so `RENDERED.md`, `RENDERED_FAST.md` and `docs/FLAGS.md` cannot go stale between edit and release. Silent when nothing drifted; one line when it heals. Inert outside `~/.claude`, skips regen's own outputs so it cannot re-trigger itself, and never blocks an edit. It never re-locks the catalog — the lock records that a human read a description, so auto-relocking would rubber-stamp drift nobody reviewed; it runs `catalog_lock.py --check` instead and prints the relock command for you to run.
...
- **secret-scan.sh** — the single leak scanner, over two rule files: `hooks/secret-patterns.txt` (credential formats, the default) and `hooks/infra-patterns.txt` (private LAN / internal-infra values, selected with `--infra`). Called by the pre-commit hook (step 2 credentials, step 3 `--infra`), verify.sh (checks 8/8a/8b, both ways) and /health (credentials only — RFC1918 addresses are legitimate content in other repos). Lints whichever file it loads, then greps stdin or files. Fails closed: a missing, empty, gutted or uncompilable rule file blocks the commit and turns verify.sh red rather than reporting clean. Nothing else may read either rule file — one engine only, which is why there is no mode that prints the joined pattern.
