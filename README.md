...
Seven outside tools make some steps better and none of them is required: every file that
reaches for one carries a fallback. `INSTALL.md` lists what each one gives you and holds the
single command that installs all seven.
...
Remove one tool at a time: `npm uninstall -g agnix ctxlint cxpak` ·
