...
A skill is a shortcut command you type, like `/health`. There are 33 custom commands, plus 1 utility command (`/memory-compile`, which lives in `commands/`), plus 11 borrowed from other people's plugins.
...
| route-map | Proves every page's content | last-30 | Trending last 30 days |
