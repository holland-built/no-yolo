...
Every run writes the table to the repo, so a search done once does not get done twice.

```bash
mkdir -p research
```

Write the whole Step 4 block to `research/<topic-slug>-<today>.md`. The slug is the topic
in lower case with spaces turned into hyphens. Add one line at the top of the file naming
the topic and the window.

Then add one line to `research/README.md`, newest first, creating that file if it is missing:

```
- [<topic>](<file>.md) — <date> — <the bottom line in under 12 words>
```

Say the file path once when you finish. Do not print the table twice.
