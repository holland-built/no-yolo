...
Helpers tier: skills under `## Helpers` in CATEGORIES.md are called by other skills (antislop by /health, apple-design by /design) — hidden from the fast menu, shown labeled in deep. `tdd` sits in that tier as a standalone red/green loop, not because anything invokes it — `/build` inlines its own compressed copy rather than calling it.
