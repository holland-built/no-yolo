...
review|Got a diff to merge or a codebase that's collected junk? `/review` is the single command. Default: reads the diff — correctness bugs, bloat, Karpathy surgical filter, plus secret scan and antislop on any .md changes, all automatic — then offers to escalate to a full codebase health pass. `--health` skips straight to fallow + trim without the diff step. `--fix` applies Critical and Important findings. `--comment` posts inline PR comments. One command replaces everything code-review and code-health used to do separately.
...
rel:review|Default mode loads CODE_REVIEW.md then fans out three parallel passes on the git diff (correctness/reuse, trim-review over-engineering, Karpathy surgical+simplicity filters), plus auto secret scan and antislop on any changed .md files. Health mode adds fallow dead-code/dupes/health/security/audit + trim-audit + trim-debt + improve — all gated between phases with optional fallow fix. `--fix` edits the working tree for diff findings; `--comment` posts to the PR via gh. Secrets are always surfaced, never auto-applied.
