...
design-full|Two speeds, one command. `/design-full --fast [surface]` fans out 7 mockups at once — 5 serious redesigns and 2 wild ones — kills the generic ones with a slop judge, opens them in Chrome, hard stop on a pick. No code. Ready to ship? `/design-full [surface]` runs the whole thing — audit, 6-persona debate, 7 Opus mockups, token extraction, Opus plan, chains to /build. Four hard gates, nothing built until you approve a mockup. Add `--steal [url]` to pull tokens from a real site you admire first — the stolen palette becomes the seed.
token-hunt|You want your site to feel like a site you admire, but you don't know how to describe why. `/token-hunt [your-url]` figures out what kind of site you have, finds 5 real sites with the same vibe, pulls their actual CSS tokens — colors, fonts, spacing — and shows you a table. Pick one. That token set goes straight into `/design-full` as the palette seed so the mockups are grounded in something real, not a random color generator.
design-fix|One button in the wrong place, one nav that feels clunky? `/design-fix [describe it]` generates 7 structural variants of that one thing — same colors, same fonts, just different layouts. Five sensible options, two wild ones. Hard stop after you pick. Nothing else changes.
