...
worktree|Asked for a worktree before and had the work happen in the main checkout anyway? `/worktree [name]` fixes the two ways that breaks — it echoes `git rev-parse --show-toplevel` and refuses to edit until the root shows `/.worktrees/`, hands every subagent the worktree's absolute path (a subagent can't inherit the cwd move), and ends by proving the diff is on the worktree branch with the main checkout clean. Gates you can watch pass, not a reminder that gets ignored.
