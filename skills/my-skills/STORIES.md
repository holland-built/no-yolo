...
debate|Not sure which option is actually better? `/debate [decision]` convenes your product team — Senior Dev, Junior Dev, Sales Engineer, DevOps, Sales Leader, Eng Leader — each pushing their real-world agenda. It maps where they contradict each other, synthesizes a briefing, then peer-reviews its own findings. Use it for architecture calls, UI/UX choices, or deciding what to build next when you want pushback, not a rubber stamp.
