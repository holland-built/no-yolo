...
worktree|Asked for a worktree before and had the work happen in the main checkout anyway? `/worktree [name]` makes it mechanical — creating the worktree arms a hard PreToolUse hook (`worktree-guard.js`) that outright DENIES any edit to the repo's main checkout until you land or cancel, the same real-deny posture as `/lockstep`. It protects every session touching that repo, even a different Orca card's agent that never loaded the skill. `/worktree land` merges the branch into base, pushes, removes the worktree, and disarms the guard; `/worktree off` cancels without merging. Not a reminder that gets ignored — a block the model can't talk past.
