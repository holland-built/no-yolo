...
research-improve|Want improvement ideas grounded in what's actually trending, not just static code smells? `/research-improve` pulls the last 30 days of movement in your repo's domain via `/last-30`, then hands those trends to `/improve` so its plans account for current best-practice shifts. Plan-only and advisory — the lightweight sibling of `/review` (no fallow/trim/secret pass, no edits). Name a topic or let it infer one from the repo.
