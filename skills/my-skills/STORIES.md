...
md-fix|`/md-check` tells you what's wrong with your docs but never fixes it — `/md-fix` does. It runs the same audit, then proposes a fix for each finding (dedupe a repeated rule, merge two overlapping files, trim an oversize one, correct a stale skill description), shows you the list, and applies them all after you say go. Add `--auto` to skip the gate. Never touches memory/, plans/, or settings.
