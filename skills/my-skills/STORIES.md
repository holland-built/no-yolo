...
worktree|Asked for a worktree before and had the work happen in the main checkout anyway? `/worktree [task]` makes it mechanical — creating the worktree arms a hard PreToolUse hook (`worktree-guard.js`) that outright DENIES any edit to the repo's main checkout, the same real-deny posture as `/lockstep`, then does the requested work inside it. It protects every session touching that repo, even a different Orca card's agent that never loaded the skill. When you say "release" or "done", the branch merges into base and the worktree is removed automatically — no sub-commands to remember.
