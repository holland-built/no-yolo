...
| design | vendored taste-skill, **emil-design-eng** (motion only), slop validator, **trim** (3+ new components only), Chrome (headless) | fresh generation: brand seed → 10 Opus mockups → validator → pick → Opus plan → Sonnet build → reuse/simplicity gate | Starting a new design or full redesign | `/design [text/URL/screenshot]` | Never preserves the existing design — every mockup is clean-sheet |
| design-audit | Lazyweb MCP, shadcn/ui MCP, Taste/Swiss/UIwiki sub-skills, **review-animations** (motion only), **Paper MCP** (fix pass, single-component targets only) | screenshot + 5 parallel lenses → violations table + top-10, light+dark | UI feels off, want a diagnosis | `/design-audit [surface]` | Five independent lenses catch what one reviewer misses — read-only, zero risk |
