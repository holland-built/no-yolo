...
| design-full | /design-audit, /debate, Lazyweb (lite --fast / deep full), Interface Design MCP, Design+Refine MCP, Sonnet agents ×7 (--fast) / Opus agents ×7 (full), /build | --fast: 7 Sonnet mockups → pick gate; full: audit → debate → 7 Opus mockups → token extract → plan → build | Quick mockups or full redesign to ship | `/design-full --fast [surface]` or `/design-full [surface]` | Two modes, one trigger — fast stops at pick, full builds nothing without 4 approved gates |
