...
| worktree | Creates a git worktree and arms a hard hook that blocks edits to the main checkout until you land; `land` merges to base + removes the worktree, `off` cancels. | You asked for work in a worktree before and it ended up in the main checkout anyway | A hard PreToolUse hook denies edits to the repo's main checkout while active — not a prompt the model can talk past — and protects every session, even a different Orca card's agent |
