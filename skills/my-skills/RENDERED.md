...
| worktree | Creates a git worktree and guarantees all work + subagent edits land inside it — show-toplevel gate before editing, branch+diff proof at the end. | You asked for work in a worktree before and it ended up in the main checkout anyway | Gate A echoes show-toplevel before any edit, subagents get the worktree abs path, Gate B proves the diff is on the branch with main clean |
