...
| debate | 6-persona product-team debate → contradiction map → one decisive verdict. | Architecture calls, UI/UX choices, or "what should we build next" when you want real pushback | Ends with YES/NO/CONDITIONAL and the one reason that settles it — no "it depends" |
...
| md-fix | Audits then APPLIES the fixes md-check only reports: dedupe, merge overlaps, trim oversize, fix drift — behind one approve-all gate (--auto skips it). | md-check flagged problems and you want them fixed, not just listed | Closes the loop md-check leaves open — one command audits and applies, instead of you hand-editing every finding |
