...
- The hook blocks `Edit`/`Write`/`NotebookEdit` and destructive `Bash`, judged per command segment so `npm test && rm -rf dist` is caught on the second half. Ordinary Bash passes — a hold you can't work inside gets switched off and stays off.
