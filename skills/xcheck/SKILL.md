---
name: xcheck
description: Use this skill when the user types /xcheck, says 'bounce this off codex', 'cross-check this plan', or 'second model opinion'. Sends a plan/diagnosis/decision to OpenAI Codex for critique, Claude adjudicates each finding, patches the artifact, max 2 rounds. Also invoked internally by /build --plan-only, /debate, /build, /health --debate, /design audit mode.
user-invocable: true
effort: high
...
- A critique round takes 1-3 minutes, so set Bash timeout ≥ 310000 (above the runner's own 300s) and never run it backgrounded-and-forgotten inside a host skill gate.
