...
5. **Pick a named macrostructure per variant, and rotate.** Read the slim index at `vendor/hallmark/skills/hallmark/references/macrostructures.md` — one line per macro, 21 of them — then open **exactly one** file from `vendor/hallmark/skills/hallmark/references/macrostructures/` per variant. Never load the whole catalogue; the index exists so you don't. A macrostructure is a whole page-shape (heading placement, body composition, divider language, button voice, image treatment, reveal) bundled as one named choice, which is why it produces more variety than picking six axes freehand. **State the pick in plain text before writing any mockup**, and rotate: read the `/* Hallmark · macrostructure: … */` stamp if a previous run left one in this project's CSS, and pick something else. The rotation rule itself is at `vendor/hallmark/skills/hallmark/SKILL.md` § "Theme-diversification rule (mandatory)" — read that heading, not the file. Do **not** create `.hallmark/log.json`; upstream's rule writes that state into the project root and `/design` does not, unless the owner asks and the project's `.gitignore` is checked first. Vendored from `Nutlope/hallmark`, MIT, pinned, gitignored — on a machine that never vendored it the folder is absent, so skip silently and set direction from `TASTE_CORE.md` Section 3 alone. **Reference only: never invoke hallmark as a skill or route a design request to it** — `/design` is the door, and its four verbs are not routes.
