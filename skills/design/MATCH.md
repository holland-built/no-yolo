...
- **ELEVATE** a too-basic sibling: if it lacks the golden's structural pattern (missing states, no spacing rhythm, flat where the golden is layered, with fewer structural elements than the golden's pattern), rebuild it to the golden's structure, then pour in the sibling's OWN real content.
