...
A skill's `description:` in its SKILL.md is the TRUTH. Up to 6 hand-maintained rows describe
that same skill (`STORIES`, `TAGLINES`, `TAGLINES_SHORT`, `WHEN_TO_USE`, `WHY_TO_USE` +
`docs/SKILL_TRIGGERS.md`). ~186 strings duplicate 27 descriptions, so drift is structural:
edit one side, the other silently lies. (`RENDERED*.md` are generated by `regen.py` — never
check or edit them; fix the sources and regen.)

**Do NOT re-read all six files and eyeball them.** That was the old design and it failed: one
agent judging 40 skills × 6 files samples instead of enumerating — two runs returned two
