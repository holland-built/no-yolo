...
**Do NOT re-read every catalog file and eyeball them** — one agent judging the whole library samples instead of enumerating (two runs once returned disjoint findings).
...
**Bound the work**: one skill at a time, or subagents at **≤7 skills each** — never one agent over the whole library.
...
Two-way check: dangling references to skills that no longer exist, and real skills nobody can discover.
...
Audit → fix table → one approve-all gate → apply → re-verify.
